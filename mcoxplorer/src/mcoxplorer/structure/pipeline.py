...
def run_structure_module(cfg: dict, sequence_features: pd.DataFrame | None = None) -> dict[str, pd.DataFrame]:
    ...
    for cid in all_candidate_ids:
        ...
        if cid in qc_by_id.index:
            ...
            qcfg = structure_cfg["quality"]
            if not bool(qrow.get("structure_quality_pass", False)):
                quality_penalty = float(qcfg["penalty_if_quality_fail"])
            if pd.notna(qrow.get("structure_coverage")) and qrow.get("structure_coverage") < qcfg["min_structure_coverage"]:
                quality_penalty = max(quality_penalty, float(qcfg["penalty_if_low_coverage"]))
            if pd.notna(qrow.get("mean_structure_confidence")) and qrow.get("mean_structure_confidence") < qcfg["min_structure_confidence"]:
                quality_penalty = max(quality_penalty, float(qcfg["penalty_if_low_confidence"]))
